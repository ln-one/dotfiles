...
### 前置要求
在开始之前，请确保已安装 Homebrew：

```bash
# macOS 和 Linux 统一使用 Homebrew 包管理器
...
├── run_once_install-homebrew.sh.tmpl         # Homebrew 安装 (可选，推荐手动安装)
├── run_onchange_install-brew-packages.sh.tmpl # 统一的 Homebrew 包管理
...
本项目采用 **Homebrew 统一包管理** 策略，在所有支持的平台上提供一致的工具安装和管理体验。

### 核心优势
- **跨平台一致性**: macOS 和 Linux 使用相同的工具版本
- **简化维护**: 单一的 Brewfile 配置文件管理绝大部分工具
- **版本统一**: 避免系统包管理器版本差异导致的兼容性问题
- **现代工具**: 优先使用现代化的 CLI 工具替代传统工具

### 例外情况
- **1Password CLI**: Linux 上必须通过 apt 仓库安装，macOS 通过 Homebrew
- **系统特定工具**: 某些平台特定的工具仍需要系统包管理器

### 工具映射
```bash
# 传统工具 → 现代工具 (通过 Homebrew 安装)
ls       → eza          # 现代化文件列表工具
cat      → bat          # 语法高亮的文件查看器
find     → fd           # 快速文件搜索工具
grep     → ripgrep      # 高性能文本搜索工具
cd       → zoxide       # 智能目录跳转工具
```

### 包管理流程
1. **用户手动安装 Homebrew** (一次性设置)
2. **系统通过 Brewfile 管理所有工具** (自动化)
3. **统一的更新和维护** (`brew upgrade`)
...
### 工具管理
- **统一包管理**: 所有工具通过 Homebrew 统一安装和管理
- **跨平台一致性**: macOS 和 Linux 使用相同的工具版本和配置
- **现代工具**: eza, bat, fd, ripgrep, fzf, zoxide, jq 等现代 CLI 工具
- **开发环境**: git, curl, neovim, tmux 等开发工具通过 Brewfile 自动配置
...
- **统一包管理**: 完全基于 Homebrew，摒弃系统包管理器，确保跨平台一致性
...
### 支持的平台
- ✅ **Ubuntu 24.04 LTS** - 完整支持，通过 Homebrew 统一包管理
- ✅ **macOS 12.0+** - 完整支持，原生 Homebrew 集成
- ✅ **SSH 远程服务器** - 优化的远程环境配置，统一工具版本
...
- **[Homebrew 迁移指南](docs/homebrew-migration-guide.md)** - 从旧系统迁移到统一包管理
